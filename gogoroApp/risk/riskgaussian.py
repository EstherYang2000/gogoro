...
# [lat,long,distance]
    ...
    def change3826to4326(self,coordinates_list):
        ...
        for x, y in coordinates_list:
            ...
            lon_list.append(lon)
            lat_list.append(lat)
        lonlat4326_list = self.create_longlat(lon_list,lat_list)
        return lonlat4326_list
    
    def swapinout(self,longlat_list,distance_list,output_file):
    ...
    def setboundingbox(self,latitude,longitude):
        ...
        distances = [NP.sqrt((lon - center[0]) ** 2 + (lat - center[1]) ** 2) for lon, lat in zip(longitude, latitude)]
    ...
    def create_boundingBox(self, longitude, latitude):
        ...
        points = [Point(point) for point in longlatlist]

        geo_s = gpd.GeoSeries(points)

        return shapely.geometry.box(*geo_s.total_bounds).buffer(3)
        
    def execute(self, scooter_id):
        data = pd.read_csv('./XYdis_10users_30times_max.csv')
        userdata = data[data['scooter_id']== scooter_id]
        # parameter setting
        longitude_swapin = userdata['swapIn_X'].values.tolist()   #longitude_swapin
        latitude_swapin = userdata['swapIn_Y'].values.tolist()   #latitude_swapin
        longitude_swapout = userdata['swapout_X'].values.tolist()   #longitude_swapout
        latitude_swapout = userdata['swapout_Y'].values.tolist()   #latitude_swapin
        ...
        longlatswapin_list = self.create_longlat(longitude_swapin, latitude_swapin) 
        longlatswapout_list = self.create_longlat(longitude_swapout, latitude_swapout)
        ...
        scale = 0.1
        filepath_swapingeojson = f'/data/workspace_g6/test/output/{scooter_id}_swapin.geojson'
        print(filepath_swapingeojson)
        filepath_swapoutgeojson = f'/data/workspace_g6/test/output/{scooter_id}_swapout.geojson'
        print(filepath_swapoutgeojson)
        filepath_raster = f'/data/workspace_g6/test/output/{scooter_id}_raster.tif'
        print(filepath_raster)
        self.swapinout(longlatswapin_list,trip_1_distance_list, filepath_swapingeojson)  #swapin buffer
        self.swapinout(longlatswapout_list,trip_last_distance_list, filepath_swapoutgeojson)  #swapout buffer
        
        #swap in raster
        self.update_params(trip_1_distance_list, trip_1_distance_list,theta_list,scale)
        self.fit(longlatswapin_list, y)    
        zRaster,yRaster = self.to_2D(xmin, xmax, ymin, ymax, cellsize=50, n_cell_per_job=1000, tqdm=False, crs='EPSG:3826', to_raster = filepath_raster)   
        ...
        print(yRaster)
    """
    def p(self,xmin, xmax, ymin, ymax):
        z_new = self.predict([217657.06944529,2507953.06529965])
        print("z_new:")
        print(z_new)
    """
    def get_risk_coefficient(self):
        
        car_acc_df = pd.read_parquet('A1A2data.parquet')
        print(car_acc_df)
        
        query_points = car_acc_df[(car_acc_df['lng'] >= self.xmin) & (car_acc_df['lng'] <= self.xmax) &
                         (car_acc_df['lat'] >= self.ymin) & (car_acc_df['lat'] <= self.ymax)][['lng', 'lat']]
        
        print("-----query_points-----")
        print(query_points)

        indices = KDTree(car_acc_df[['lng', 'lat']]).query(query_points)

        # 預測機率值並計算風險係數
        risk_coefficient = 0
        for i in range(len(query_points)):
            lng, lat = query_points.iloc[i]
            prediction = self.predict([lng, lat])
            risk_coefficient += prediction * car_acc_df.iloc[indices[i]]['danger_val']
        
        print("-----risk_coefficient-----")
        print(risk_coefficient)
...
def main():
    test = gaussian()
    scooter_id = '03a6729f-8045-4ada-9110-3ec91079c83c'
    test.execute(scooter_id)
    test.get_risk_coefficient()
    #set parameter
    #scooter_id = ''
